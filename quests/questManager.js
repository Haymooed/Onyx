...
class QuestManager {
    ...
    log(questId, msg) {
        let prefix = '';
        if (typeof questId === 'string' && msg !== undefined) {
            const q = this.get(questId);
            const name = q ? (q.config.messages?.quest_name || q.config.application?.name || questId) : questId;
            prefix = `[${name}] `;
        } else {
            msg = questId;
        }
    ...
    sleep(ms) {
        return new Promise((resolve, reject) => {
            ...
            const cancel = () => { clearTimeout(timer); reject(new Error('Stopped')); };
    ...
    async doingQuest(quest) {
        ...
        if (playTask) await this.runPlay(quest, tasks[playTask], playTask);
        else if (videoTask) await this.runVideo(quest, tasks[videoTask], videoTask);
        else this.log(quest.id, 'Unsupported Task Type.');
    }

    async runVideo(quest, taskConfig, taskName) {
        this.log(quest.id, 'Task: WATCH_VIDEO');
        const speed = 7, target = taskConfig.target;
        ...
        while (current < target && !this.stopped) {
            ...
            try {
                ...
                if (res.completed_at) { this.log(quest.id, 'COMPLETED!'); quest.updateUserStatus(res.user_status); break; }
        ...
        if (current >= target && !this.stopped) {
            try { const res = await this.videoProgress(quest.id, target); if (res.completed_at) this.log(quest.id, 'COMPLETED!'); } catch (e) {}
        }
    }
    ...
    async verifyProgress(questId) {
        try {
            ...
            return res.quests.find(q => q.id === questId)?.user_status?.progress;
        } catch (e) { return null; }
    }
    ...
    async runPlay(quest, taskConfig, taskName) {
        ...
        while (current < target && !this.stopped) {
            try {
                ...
                if (current >= target) {
                    this.log(quest.id, 'Target reached. Verifying...');
                    ...
                    if (realValue !== undefined && realValue < target) {
                        this.log(quest.id, `Correction: Real progress ${realValue}/${target}s. Continuing...`);
                        current = realValue;
