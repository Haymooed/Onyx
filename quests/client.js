...
class ClientQuest extends Client {
    constructor(token) {
        ...
        this.websocketManager.on('error', (err) => {
            // Silently ignore gateway errors (DNS failures after disconnect, etc.)
        });
    }
    ...
    async destroy() {
        try {
            const shardIds = await this.websocketManager.getShardIds();
            for (const id of shardIds) {
                await this.websocketManager.destroy(id).catch(() => {});
            }
        } catch {}
    }
