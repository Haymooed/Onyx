...
async function makeRequest(url, init) {
    if (init.headers) {
        ...
        if (myHeaders.has('User-Agent')) myHeaders.set('User-Agent', USER_AGENT);
...
class ClientQuest extends Client {
    constructor(token) {
        ...
        const gateway = new WebSocketManager({ token, intents: 0, rest });

        gateway.fetchGatewayInformation = () => Promise.resolve({
    ...
    async fetchQuests() {
        ...
        return QuestManager.fromResponse(this, response);
    }
}
